// Mock-Datenbank für realistische Test-Daten
...
class MockDatabase {
  ...
  private generateInvoice(): InvoiceData {
    ...
    return {
      ...
      status: faker.helpers.arrayElement(['completed', 'processing', 'error'] as const),
      ...
      due_date: faker.date.future({ days: 30, refDate: invoiceDate }).toISOString().split('T')[0],
      ...
      skr03_account: account.code,
      booking_text: this.generateBookingText(supplier, account),
      confidence_score: faker.number.float({ min: 0.8, max: 1.0, fractionDigits: 2 }),
      extraction_errors: Math.random() > 0.9 ? [faker.lorem.sentence()] : [],
  ...
  getInvoices(page: number = 1, limit: number = 50): { invoices: InvoiceData[], pagination: any } {
  ...
  getBookings(page: number = 1, limit: number = 50): { bookings: BookingData[], totals: any, pagination: any } {
    const start = (page - 1) * limit;
    const end = start + limit;
    ...
    const totalVat = this.bookings.reduce((sum, booking) => sum + (booking.vat_amount || 0), 0);
  ...
  addUpload(files: File[]): { results: UploadResult[] } {
  ...
  private createInvoiceFromUpload(upload: UploadResult): InvoiceData {
    ...
    return {
      ...
      skr03_account: account.code,
      booking_text: this.generateBookingText(supplier, account),
      confidence_score: faker.number.float({ min: 0.85, max: 0.98, fractionDigits: 2 }),
      extraction_errors: [],
